...
Usage: R/knn2_testing.R --test_data=<test_data> --out_dir=<out_dir>
Options:
...
main <- function(test_data, out_dir){
  ...
  # Load knn model
  knn_model_2 <- readRDS("results/knn_model_2.rds")
  
  # Find RMSPE
  knn_rmspe_2 <- model_rmspe(knn_model_2, bike_testing, "bike_count")
  
  # Save RMSPE value
  saveRDS(knn_rmspe_2, file = paste0(out_dir, "/knn_rmspe_2.rds"))
  
  # create plot
  knn_prediction_2 <- knn_model_2 |>
    stats::predict(bike_training) |>
    dplyr::bind_cols(bike_training)
  
  knn_bike_count_solar_rad <- scatter_plot(knn_prediction_2, 
                                      solar_radiation, 
                                      bike_count, 
  ...
  # Save plot
  ggsave(paste0(out_dir, "/knn_bike_count_solar_rad.png"), knn_bike_count_solar_rad)
}
